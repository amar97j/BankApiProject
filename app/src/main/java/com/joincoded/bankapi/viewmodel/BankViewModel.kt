...
class BankViewModel : ViewModel() {
    ...
    fun signin(username: String, password: String, image: String = "") {
        viewModelScope.launch {
            try {
                val response = apiService.signin(User(username, password, image, null))
                token = response.body()
            } catch (e: Exception) {
                println("Error $e")
            }
    ...
    fun transfer(username: String, password: String,amountChange: AmountChange) {
        viewModelScope.launch {
            try {
                val response = apiService.transfer(username, password,amountChange)
                token = response.body()
            } catch (e: Exception) {
                println("Error $e")
            }
